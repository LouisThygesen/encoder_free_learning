...
def hyperparameters():
    ...
    argparser.add_argument('-sl', type=int, default=2)
    argparser.add_argument('-theta_prior', type=int, default=0)
    argparser.add_argument('-lat_dim', type=int, default=16)
    
    args = argparser.parse_args()

    # Create hyper-parameter object
    class config():
        def __init__(self,data,epochs,opt_mlp,opt_zs,lr_mlp,lr_zs,wd_mlp,sl,theta_prior,lat_dim):
            self.data = data
            self.model = "HDGD"
            self.epochs = epochs
            self.train_bs = 32
...
def build_optimizers(config, model):
    ...
    if (config.opt_mlp == "ADAM") and (config.opt_zs == "ADAM"):
        for i in range(config.sl):
            train_opts += [torch.optim.Adam(params=model.decoder[i].parameters(), lr=config.lr_mlp,
                                            weight_decay=config.wd_mlp)]
            train_opts += [torch.optim.Adam(params=[model.zs_train[i]], lr=config.lr_zs)]
            test_opts += [torch.optim.Adam(params=[model.zs_test[i]], lr=config.lr_zs)]

    if (config.opt_mlp == "ADAM") and (config.opt_zs == "SGD"):
        for i in range(config.sl):
            train_opts += [torch.optim.Adam(params=model.decoder[i].parameters(), lr=config.lr_mlp,
                                            weight_decay=config.wd_mlp)]
            train_opts += [torch.optim.SGD(params=[model.zs_train[i]], momentum=0.9, lr=config.lr_zs)]
            test_opts += [torch.optim.SGD(params=[model.zs_test[i]], momentum=0.9, lr=config.lr_zs)]

    if (config.opt_mlp == "SGD") and (config.opt_zs == "ADAM"):
        for i in range(config.sl):
            train_opts += [torch.optim.SGD(params=model.decoder[i].parameters(), momentum=0.9,
                                           lr=config.lr_mlp, weight_decay=config.wd_mlp)]
            train_opts += [torch.optim.Adam(params=[model.zs_train[i]], lr=config.lr_zs)]
            test_opts += [torch.optim.Adam(params=[model.zs_test[i]], lr=config.lr_zs)]

    if (config.opt_mlp == "SGD") and (config.opt_zs == "SGD"):
        for i in range(config.sl):
            train_opts += [torch.optim.SGD(params=model.decoder[i].parameters(), momentum=0.9,
                                           lr=config.lr_mlp, weight_decay=config.wd_mlp)]
            train_opts += [torch.optim.SGD(params=[model.zs_train[i]], momentum=0.9, lr=config.lr_zs)]
...
def train_epoch(config, model, train_loader, train_opts, epoch):
    ...
    with tqdm(train_loader, unit="batch", desc=f" {epoch+1}/{config.epochs}") as batch:
        for (x, y, idx) in batch:
            ...
            # Take optimization step (first network and then zs - starting from innermost layer)
            train_loss.backward()

            if config.opt_start == 1:
                for opt in train_opts:
                    opt.step()
            if config.opt_start == 2:
                mlp_opts = train_opts[::2][::-1]
                zs_opts = train_opts[1::2][::-1]
                
                for (mlp_opt,zs_opt) in zip(mlp_opts,zs_opts):
                    mlp_opt.step()
                    zs_opt.step()
...
def test_epoch(config, model, train_loader, test_loader, test_opts, epoch):
    ...
    for opt_step in range(config.opt_steps):
        ...
        for (x, y, idx) in test_loader:
            ...
            # Take optimization step (starting from innermost latent representation)
            train_loss.backward()
    ...
    with torch.no_grad():
        ...
        # Log prototyping plots to wandb (placed here before zs_test is reset)
        if epoch % 20 == 0: 
...
def plot_wandb(model, train_loader, test_loader):
    ...
    # Generate plot for each latent layer
    pcas = []
    plots = []
    ...
    for i in range(config.sl):
        ...
        for (x, y, idx) in train_loader:
            if count == 0:
                zs = model.zs_train[i][idx].detach().cpu()
                count += 1

            else:
                lol = model.zs_train[i][idx].detach().cpu()
        ...
        pcas[i].fit(zs)
        x = pcas[i].transform(zs)

        # Structure data and plot it
        df = pd.DataFrame(dict(x1=x[:, 0], x2=x[:, 1], y=ys, size=3, alpha=0.1))
    ...
    image_save3 = wandb.Table(data=images)
    ...
    for i in range(config.sl):
        ...
        for (x, y, idx) in test_loader:
            ...
            if count == 0:
                zs = model.zs_test[i][idx].detach().cpu()

            else:
                lol = model.zs_test[i][idx].detach().cpu()
                zs = torch.vstack((zs, lol))
...
def log_final_stats(config, model):
    ...
    vars = [torch.var(model.zs_test[i],axis=0) for i in range(config.sl)]
    activated = [torch.sum(vars[i] > 10**(-2)).detach().cpu().item() for i in range(config.sl)]
    ...
    # Log training and test epoch results
    main_log['train_loss'] = np.array(train_data['loss'])
    main_log['train_log_PxGz'] = np.array(train_data['log_PxGz'])
    main_log['train_log_PzL'] = np.array(train_data['log_PzL'])
    main_log['train_log_rest'] = np.array(train_data['log_rest'])
    main_log['val_loss'] = np.array(val_data['loss'])
    main_log['val_log_PxGz'] = np.array(val_data['log_PxGz'])
    main_log['val_log_PzL'] = np.array(val_data['log_PzL'])
    main_log['val_log_rest'] = np.array(val_data['log_rest'])
    

    main_log.to_csv(f'{config.main_path}/main_log.csv')

    # Reset test latent representations
    model.reset_zs_test()
...
def train(config):
    ...
    # Load optimizers
    train_opts, test_opts = build_optimizers(config, model)
    ...
    for epoch in range(config.epochs):
        ...
        # Test on 1 epoch every 5th epoch - in the rest epoch just copy previous result (creates staircase diagram).
        # The model is evaluated so few times as the test optimization takes a lot of time.
        if epoch % 10 == 0:     # DEBUG 5
            model.reset_zs_test()
            ...
            test_epoch(config, model, train_loader, test_loader, test_opts, epoch)
            
        else:
            val_data['loss'] += [val_data['loss'][-1]]
            val_data['log_PxGz'] += [val_data['log_PxGz'][-1]]
            val_data['log_PzL'] += [val_data['log_PzL'][-1]]
            val_data['log_rest'] += [val_data['log_rest'][-1]]
        ...
        data = {
            ...
            'train_log_PzL': train_data['log_PzL'][-1],
            'train_log_rest': train_data['log_rest'][-1],
            'val_loss': val_data['loss'][-1],
            'val_log_PxGz': val_data['log_PxGz'][-1],
            'val_log_PzL': val_data['log_PzL'][-1],
            'val_log_rest': val_data['log_rest'][-1],
        }
    ...
    for opt_step in range(config.opt_steps):
        ...
        for (x, y, idx) in test_loader:
            ...
            # Compute loss from forward pass
            loss, diagnostics = model.loss(x, idx=idx, mode="test")
...
if __name__ == "__main__":
    ...
    log_name = f"{config.model}_{config.data}_{config.sl}"
