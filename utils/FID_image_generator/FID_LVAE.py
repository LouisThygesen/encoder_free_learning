...
if args.data == "MNIST":
    path = '/zhome/71/2/146488/bachelor_project/FID_score/MNIST_LVAE'
    batches = 40
    images = 10240
    sl = 5
if args.data == "OMNIGLOT":
    path = '/zhome/71/2/146488/bachelor_project/FID_score/OMNIGLOT_LVAE'
    batches = 32
    images = 8192
    sl = 5

# Initialize model with trained weights
model = LVAE(in_features=784, layers=sl, batch_norm=1, device=device).to(device)   
...
# Load model 
if args.data == "MNIST":
    model.load_state_dict(torch.load('saved_models/model_MNIST_LVAE.pth'))

if args.data == "OMNIGLOT":
    model.load_state_dict(torch.load('saved_models/model_OMNIGLOT_LVAE.pth'))
