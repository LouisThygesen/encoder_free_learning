...
df1 = pd.read_csv('main_log_conv.csv')
df2 = pd.read_csv('main_log_lin.csv')
...
# Set limits
min_train_loss = min(df1['train_loss'].min(), df2['train_loss'].min())
max_train_loss = max(df1['train_loss'].max(), df2['train_loss'].max())
...
# Set limits
min_test_loss = min(df1['val_loss'].min(), df2['val_loss'].min())
max_test_loss = max(df1['val_loss'].max(), df2['val_loss'].max())
...
# Set limits
min_rec_loss = min(df1['train_log_PxGz'].min(), df2['train_log_PxGz'].min())
max_rec_loss = max(df1['train_log_PxGz'].max(), df2['train_log_PxGz'].max())
...
# Set limits
min_rec_loss = min(df1['val_log_PxGz'].min(), df2['val_log_PxGz'].min())
max_rec_loss = max(df1['val_log_PxGz'].max(), df2['val_log_PxGz'].max())
...
axs[3].legend(['conv','lin'], loc='upper right', fontsize=18)
