...
class DGD_CONV(nn.Module):
    def __init__(self, config, train_size, test_size):
        super(DGD_CONV, self).__init__()
        ...
        self.mlp = nn.Sequential(
            ...
            nn.Linear(30,361),
            nn.ReLU(),
            nn.Unflatten(2,(19,19)),
            nn.ConvTranspose2d(1, 64, kernel_size=(4, 4)),
            nn.ReLU(),
        ...
        # p(\phi) distributions
        self.dist1 = Dirichlet(torch.ones(10).to('cuda'))
        ...
        initializer = Uniform(low=torch.ones(10,self.latent_dim).to('cuda')*(-5),
                              high=torch.ones(10,self.latent_dim).to('cuda')*5)
    ...
    def loss(self, x, idx, mode):
        ...
        log_prob1 = self.dist1.log_prob(nn.functional.softmax(self.mixture_coeffs))
